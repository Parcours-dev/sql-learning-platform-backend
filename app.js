...
app.get('/api/top-student', async (req, res) => {
    try {
        const [results] = await db.query(`
            ...
            GROUP BY u.UserID
            ORDER BY CorrectAnswers DESC LIMIT 1;
        `);
        if (results.length > 0) {
            res.json(results[0]);
        } else {
...
app.get('/api/least-successful-student', async (req, res) => {
    try {
        const [results] = await db.query(`
            ...
            WHERE ur.IsCorrect = 1
            GROUP BY u.UserID
            ORDER BY CorrectAnswersCount ASC LIMIT 1;
        `);
        if (results.length > 0) {
            res.json(results[0]);
        } else {
...
app.get('/api/most-successful-chapter', async (req, res) => {
    try {
        const [results] = await db.query(`
            ...
            ORDER BY CorrectAnswersCount DESC LIMIT 1;
        `);
        if (results.length > 0) {
            res.json(results[0]);
        } else {
        ...
        const [results] = await db.query(`
            SELECT c.ChapitreID,
                   ...
                   (SELECT COUNT(DISTINCT UserID)
                    ...
                    WHERE ur.IsCorrect = 1
                      AND q.ChapitreID = c.ChapitreID)                              as correctUsers
            FROM Chapitre c
        `);
...
app.get('/api/last-good-responses', async (req, res) => {
    try {
        const [results] = await db.query(`
            ...
            WHERE ur.IsCorrect = 1
            GROUP BY u.UserID, q.QuestionID
            ORDER BY ur.SubmissionDate DESC LIMIT 6;
        `);
        res.json(results);
    } catch (error) {
